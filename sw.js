/* ═══════════════════════════════════════════════════════════
   SERVICE WORKER — Commune de Libin (LibinBot)
   Gestion des notifications push + cache PWA
   ═══════════════════════════════════════════════════════════ */

const CACHE_NAME = 'libinbot-v1';  /* ← à incrémenter pour forcer le rechargement sur tous les appareils */
const ASSETS = [
  '/LibinBot/',
  '/LibinBot/index.html',
  '/LibinBot/module4-prompt.js',
  '/LibinBot/module6-api.js',
  '/LibinBot/module-voice.js',
  '/LibinBot/manifest.json',
  '/LibinBot/blason-libin.png',
  '/LibinBot/icon-192.png',
  '/LibinBot/icon-512.png'
];
...
self.addEventListener('push', function(e) {
  let data = { title: 'Commune de Libin', body: 'Nouveau message de la Ville.', url: '/LibinBot/' };
  try {
  ...
  const options = {
    body:               data.body,
    icon:               '/LibinBot/icon-192.png',
    badge:              '/LibinBot/icon-192.png',
    vibrate:            isUrgent ? [200, 100, 200, 100, 200] : [100, 50, 100],
    data:               { url: data.url || '/LibinBot/' },
...
self.addEventListener('notificationclick', function(e) {
  ...
  const target = e.notification.data?.url || '/LibinBot/';
  e.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function(list) {
      for (const client of list) {
        if (client.url.includes('/LibinBot/') && 'focus' in client) {
          return client.focus();
        }
      }
